// Healthcheck Agent v3 — Thinking + Tool Use + Agent Loop
// Propósito: healthcheck autónomo del cluster kind con razonamiento
//            profundo activado (Thinking). A diferencia de v2 (Haiku),
//            esta versión usa Sonnet con budget de thinking para que
//            Claude razone antes de decidir qué tool invocar.
// Autor: Erick Diaz
// Fecha: 2026-08-15
// Ambiente objetivo: lab WSL2 — kind cluster sre-lab
...
// ── DEFINICION DE TOOLS (igual que v2) ──────────────────────────────────────
...
// ── EJECUTORES DE TOOLS (igual que v2) ──────────────────────────────────────
const incidents: any[] = [];
...
async function runAgentLoop(): Promise<string> {
  ...
  while (true) {
    ...
    // ── LA DIFERENCIA CON v2: thinking activado + Sonnet + max_tokens alto ──
    const response = await client.messages.create({
      model: "claude-sonnet-4-6",
      max_tokens: 16000,          // obligatorio cuando thinking está activo
      thinking: {
        type: "enabled",
        budget_tokens: 8000,      // Claude puede usar hasta 8000 tokens para razonar
      },
      tools,
      messages,
    });
    ...
    if (thinkingBlock && thinkingBlock.type === "thinking") {
      ...
      console.log("  " + thinkingText.slice(0, 500) + (thinkingText.length > 500 ? "..." : ""));
    }

    // Agregar respuesta al historial
    messages.push({ role: "assistant", content: response.content });

    // Claude terminó — no necesita más tools
    ...
    if (response.stop_reason === "tool_use") {
      ...
      for (const block of response.content) {
        if (block.type === "tool_use") {
          console.log(`\n  Claude pide tool: ${block.name}`);
          console.log(`  Parámetros: ${JSON.stringify(block.input)}`);
...
function saveReport(summary: string): void {
  ...
  const filepath = `/tmp/healthcheck-v3-${timestamp}.md`;

  let report = `# Healthcheck v3 — Thinking + Tool Use + Agent Loop\n`;
  report += `**Modelo:** claude-sonnet-4-6 con Thinking (budget: 8000 tokens)\n`;
...
// ── MAIN ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log("=== Healthcheck Agent v3 — Thinking + Tool Use + Agent Loop ===");
  console.log(`Modelo: claude-sonnet-4-6 | Thinking budget: 8000 tokens`);
